...
export default function POS(){
  ...
  async function payWithUPI(){
    ...
    } catch (err){ console.error(err); alert('Error initiating UPI payment') }
  }
        ...
        <div className="product-grid">
          {products.length === 0 ? (
            ...
            <div key={p._id} className="product">
              <img src={p.imageUrl || 'https://via.placeholder.com/400x300?text=No+Image'} alt={p.name} style={{width:'100%',height:160,objectFit:'cover',borderRadius:8}} />
              <h4>{p.name}</h4>
